...
import { useTable, useUpdate, useDelete, useNavigation, useGetIdentity } from "@refinedev/core";
...
export const VendorsList = () => {
  ...
  const { data: identityData } = useGetIdentity<Identity>();
  ...
  // Ten admin może edytować TYLKO własną organizację (zgodne z docelową polityką RLS)
  const canEditVendor = (vendor: Vendor) => isAdmin && vendor.id === me?.vendor_id;
  // Tworzenie/usuwanie vendorów — wyłączone (brak polityk RLS na INSERT/DELETE)
  const canCreateVendor = false;
  const canDeleteVendor = (_vendor: Vendor) => false;

  const handleToggleActive = async (vendor: Vendor) => {
    if (!canEditVendor(vendor)) return;
    await updateVendor({
      resource: "vendors",
      id: vendor.id,
      values: { is_active: !vendor.is_active },
      successNotification: () => ({
  ...
  const handleDeleteClick = (vendor: Vendor) => {
    if (!canDeleteVendor(vendor)) return;
  ...
  const handleConfirmDelete = async () => {
    ...
    await deleteVendor({
      ...
      successNotification: () => ({ message: "Organizacja została usunięta", type: "success" }),
      errorNotification: () => ({
        message: "Brak uprawnień do usunięcia organizacji",
        type: "error",
      }),
    });
  ...
  return (
    <SubPage>
      <FlexBox>
        <Lead title="Organizacje" description="Zarządzaj organizacjami (wg uprawnień)" />
        {canCreateVendor && (
              ...
              setFilters([
                {
                  ...
                  value: [
                    ...
                    { field: "subdomain", operator: "contains", value: e.target.value },
                  ],
                },
              ]);
            }}
      ...
      <GridBox>
        {data?.data?.map((vendor) => {
          const _canEdit = canEditVendor(vendor);
          const _canDelete = canDeleteVendor(vendor);
                ...
                <div className="flex items-center gap-2 pt-3 border-t">
                  <div className="flex items-center gap-2">
                    ...
                    <Switch
                      ...
                      onCheckedChange={() => _canEdit && handleToggleActive(vendor)}
                      disabled={!_canEdit}
                    />
                  </div>
                  ...
                  <div className="ml-auto flex gap-1">
                    ...
                    <Button variant="ghost" size="sm" onClick={() => _canEdit && edit("vendors", vendor.id)} disabled={!_canEdit}>
